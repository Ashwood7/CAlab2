...
char read_path[] = "/home/arch_stu2019012341/yao-archlab-s21/lab1/sample/turing.pixel";
char write_path[] = "/home/arch_stu2019012341/yao-archlab-s21/lab1/submit/turing_c.pixel";
//------------------------------------------------------------------------------------------------------------------
    ...
    for (int i = 0; i < n; i ++){
        for (int j = 0; j < m; j++){
            if (i == 0 || j == 0 || i == n - 1 || j == m - 1){
                result_img[i * m + j] = img[i * m + j];
            }
            else{
                result_img[i * m + j] = (img[(i-1)*m + (j-1)]
                                    + 2*img[(i-1)*m + j]
                                    + img[(i-1)*m + (j+1)]
                                    + 2*img[i * m + (j-1)]
                                    + 4*img[i * m + j]
